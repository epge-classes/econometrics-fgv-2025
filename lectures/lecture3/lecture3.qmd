...
```{=latex}
\begin{definition}[Stochastic Process -- a bit informal]
A stochastic process is a ordered family of random variables $\{Y_t , t \in \mathcal{T}\}$ defined on a probability space $(\Omega, \mathcal{F}, \mathbb{P})$, where $\mathcal{T}$ is an ordered set. One realization of this process is denoted by $\{Y_t(\omega), \omega \in \Omega\}$.
\end{definition}
```
...
- Think about the history of the world: it is a single realization from $\Omega$;
...
- In words: the maximum value of the process is not positive;
...
# White Noise and Martingale Difference Sequences

## White Noise

- In the cross-section context, it's commom to write $y_i = \alpha + \beta x_i + u_i$ where $u_i$ is i.i.d;
- We will need more flexible assumptions on "error terms" now;
- Two very important weaker notions of "error term" are **white noise** processes and **martingale difference sequences**.

. . .

```{=latex}
\begin{definition}[White Noise Process]
A sequence of random variables $(\varepsilon_t)_{t \in \mathbb{Z}}$ is called a \textbf{white noise process} if:
\begin{enumerate}
    \item $\mathbb{E}[\varepsilon_t] = 0$ for all $t$;
    \item $\operatorname{Var}(\varepsilon_t) = \sigma^2 < \infty$ for all $t$;
    \item $\operatorname{Cov}(\varepsilon_t, \varepsilon_s) = 0$ for all $t \neq s$.
\end{enumerate}

In words, a white noise process is a sequence of uncorrelated random variables with mean zero and constant variance.
\end{definition}
```
...
- Conditioning on past information is common in economic models and in Econometrics;
- The correct mathemtical object to use here are $\sigma$-**fields** and **filtrations**;
- We will avoid measure-theoretic formalism right now;
- Intuitively: the $\sigma$-field (or -algebra) generated by $(Y_t, Y_{t-1}, Y_{t-2}, ...)$ is the collection of possible histories for this process up to $t$;
- We denote this in two equivalent ways: $\mathcal{F}_t = \sigma(Y_t, Y_{t-1}, Y_{t-2}, ...)$;
- In other areas of Economics, we tipically refer to $\sigma$-fields as _information sets_;
- $\mathcal{F}_t$ contains all information available up to time $t$;
- Example: $\mathbb{E}[Y_t | \mathcal{F}_{t-1}]$ is our best guess for $Y_t$ conditional on information from this process, and only that, up to $t-1$;

## Martingale Difference Sequences

```{=latex}
\begin{definition}[Martingale Difference Sequence]
A sequence of random variables $(\varepsilon_t)_{t \in \mathbb{Z}}$ is called a \textbf{martingale difference sequence} (MDS) with respect to the information set $\mathcal{F}_{t-1}$ if:
\begin{enumerate}
    \item $\varepsilon_t$ is adapted to $\mathcal{F}_t$;
    \item $\mathbb{E}[|\varepsilon_t|] < \infty$ for all $t$;
    \item $\mathbb{E}[\varepsilon_t | \mathcal{F}_{t-1}] = 0$ almost surely for all $t$.
\end{enumerate}
\end{definition}
```
...
- Every MDS sequence has (unconditional) mean zero; (True of false?)
- $\operatorname{Cov}(\varepsilon_t, \varepsilon_{t+h}) = 0$ for all $h \neq 0$. (True of false?)
- $\mathbb{E}[\varepsilon_{t+h} | \mathcal{F}_{t}] = 0$ for all $h > 0$. (True of false?)
- $\mathbb{E}[\varepsilon_{t-h} | \mathcal{F}_{t}] = 0$ for all $h > 0$. (True of false?)


## Example

- Let $u_t$ be an i.i.d sequence with mean zero and variance $\sigma^2 < \infty$.
- Define $v_t = u_t \cdot u_{t-1}$.
    - Is this process i.i.d? Is it a MDS with respect its natural filtration (information set)? Is it white noise?

. . . 

- Now, define $m_t = u_t + u_{t-1}\cdot u_{t-2}$;
    - Is this process i.i.d? Is it a MDS with respect its natural filtration (information set)? Is it white noise?
...
## References
- Hannan, E. J. (1970). _Multiple Time Series_. Wiley.
- Chapters 3 and 4 from Hamilton's book;
