...
A \textit{stochastic process} is an ordered sequence (collection) of random variables $\{Y_t(\omega),\omega\in\Omega,t\in\mathcal{T}\}$, such that for all $t\in\mathcal{T}$, $Y_t(\omega)$ \textbf{is a random variable} and $\mathcal{T}$ is an ordering set, for example, $\mathbb{Z} = \{-2, -1, 0, 1, 2, ...\}$.
\end{definition}
```
